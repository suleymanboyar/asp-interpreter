...
public abstract class AspSyntax {
    ...
    public static void test(Scanner s, TokenKind[] tks) {
        if (!Arrays.asList(tks).contains(s.curToken().kind)) {
            String error = "Expected on of " + Arrays.toString(tks) +
                " but found " + s.curToken().kind + "!";
            parserError(error, s.curLineNum());
        }
    }
